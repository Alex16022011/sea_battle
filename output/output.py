...
from clear_variant.clear_variant_for_computer import matrix
window = Tk()
...
for i in range(12):
    for j in range(12):
        exec(f'btn{i}_{j} = Button(window, text=matrix[{i}][{j}], width=5, height=2, font="Arial 14", bg="green", activebackground="blue", activeforeground="white", command=lambda: color(btn{i}_{j}, {i}, {j}, {1}))')
